...
export function getTypeInfo(schema, localize) {
  if (!schema) {
    return;
  }
  const info = {
    ...
    readOrWriteOnly: schema.readOnly
      ...
      : schema.writeOnly
        ? localize.writeOnly
        : '',
    deprecated: schema.deprecated ? localize.deprecated : '',
  ...
  if (schema.type === 'array' || schema.items) {
    ...
    if (schema.minimum !== undefined && schema.maximum !== undefined) {
      info.constrain = `${schema.exclusiveMinimum ? '>' : `${localize.between} `}${schema.minimum} ${localize.and} ${schema.exclusiveMaximum ? '<' : ''} ${schema.maximum}`;
    ...
    if (schema.multipleOf !== undefined) {
      info.constrain = `${localize.multipleOf} ${schema.multipleOf}`;
    }
  } else if (schema.type === 'string') {
    if (schema.minLength !== undefined && schema.maxLength !== undefined) {
      info.constrain = `${schema.minLength}-${schema.maxLength} ${localize.chars}`;
    } else if (schema.minLength !== undefined && schema.maxLength === undefined) {
      info.constrain = `min:${schema.minLength} ${localize.chars}`;
    } else if (schema.minLength === undefined && schema.maxLength !== undefined) {
      info.constrain = `max:${schema.maxLength} ${localize.chars}`;
    }
  }
  info.typeInfoText = `${localize[info.type] === undefined ? info.type : localize[info.type]}~|~${info.readOrWriteOnly} ${info.deprecated}~|~${info.constrain}~|~${info.default}~|~${info.allowedValues}~|~${info.pattern}~|~${info.description}`;
  return info;
}
 ...
 */
export function schemaInObjectNotation(localize, schema, obj = {}, level = 0) {
  if (!schema) {
    return;
  }
  if (schema.type === 'object' || schema.properties) { // If Object
    ...
    for (const key in schema.properties) {
      if (schema.required && schema.required.includes(key)) {
        obj[`${key}*`] = schemaInObjectNotation(localize, schema.properties[key], {}, (level + 1));
      } else {
        obj[key] = schemaInObjectNotation(localize, schema.properties[key], {}, (level + 1));
      }
    }
    ...
    obj['::description'] = schema.description ? schema.description : '';
    obj['::type'] = 'array';
    obj['::props'] = schemaInObjectNotation(localize, schema.items, {}, (level + 1));
  } else if (schema.allOf) {
    const objWithAllProps = {};
    if (schema.allOf.length === 1 && !schema.allOf[0].properties && !schema.allOf[0].items) {
      // If allOf has single item and the type is not an object or array, then its a primitive
      const tempSchema = schema.allOf[0];
      return `${getTypeInfo(tempSchema, localize).typeInfoText}`;
    }
    // If allOf is an array of multiple elements, then all the keys makes a single object
    schema.allOf.map((v) => {
      if (v.type === 'object' || v.properties || v.allOf || v.anyOf || v.oneOf) {
        const partialObj = schemaInObjectNotation(localize, v, {}, (level + 1));
        ...
        const typeObj = getTypeInfo(v, localize);
    ...
    schema[xxxOf].map((v) => {
      if (v.type === 'object' || v.properties || v.allOf || v.anyOf || v.oneOf) {
        ...
        objWithAnyOfProps[prop] = `${getTypeInfo(v, localize).typeInfoText}`;
      }
    });
    obj[(schema.anyOf ? 'ANY:OF' : 'ONE:OF')] = objWithAnyOfProps;
  } else {
    const typeObj = getTypeInfo(schema, localize);
  ...
  for (const key in obj) {
    if (typeof obj[key] === 'object') {
      ...
      const objRow = [
        ...
        { text: localize[objType], style: ['small', 'mono', 'lightGray'], margin: 0 },
        { text: '', margin: 0 },
      ];
      allRows.push(objRow);
      if (obj[key]['::type'] === 'array') {
